...
![synology_network.drawio_EiQOJh](https://slark-blog.s3.bitiful.net/synology_network.drawio_EiQOJh.svg)
...
![synology_network.drawio3_egeUhs](https://slark-blog.s3.bitiful.net/synology_network.drawio3_egeUhs.svg)
...
![XPjbNQ_D6FmC7](https://slark-blog.s3.bitiful.net/XPjbNQ_D6FmC7.png)

### 查看当前网络

在 `控制面板` -> `网络` -> `网络界面` 中可以看到当前的网络连接状态。

![upi5hb4s.oxh_KWdA4l](https://slark-blog.s3.bitiful.net/upi5hb4s.oxh_KWdA4l.png)
...
![1mlkyale.ev2_qnqpGe](https://slark-blog.s3.bitiful.net/1mlkyale.ev2_qnqpGe.png)

下面这张图是启用 `Open vSwitch` 之后的网络配置：

![FMTgqF_epmJyh](https://slark-blog.s3.bitiful.net/FMTgqF_epmJyh.png)
...
![z1ncz00g.qok_X6wJQM](https://slark-blog.s3.bitiful.net/z1ncz00g.qok_X6wJQM.png)
...
![rzeksrx1.zvi_REiPLE](https://slark-blog.s3.bitiful.net/rzeksrx1.zvi_REiPLE.png)

现在你就可以把电脑直接接到 NAS 的万兆网卡上，电脑使用 `DHCP` 的模式就可以自动从路由器获取到 IP 上网。在电脑的网络属性中，也可以看到网络的链接速度是 `10G`。

最后，用一张图来总结下涉及到的操作。

![ovs_switch2.drawio_KwKBr1](https://slark-blog.s3.bitiful.net/ovs_switch2.drawio_KwKBr1.svg)
...
![xns9xs_7h9Fqn](https://slark-blog.s3.bitiful.net/xns9xs_7h9Fqn.png)
...
![3voWYT_5I1Xcm](https://slark-blog.s3.bitiful.net/3voWYT_5I1Xcm.png)

设置好之后点击 `确定`，出现系统提示再次 `确定` 之后输入密码提交。

![x95OJE_KboTMf](https://slark-blog.s3.bitiful.net/x95OJE_KboTMf.png)
...
![utvpGD_5Ow2NU](https://slark-blog.s3.bitiful.net/utvpGD_5Ow2NU.png)

这里把 `启用绑定` 改为 `否`，`启用桥接` 改为 `是`，在 `桥接其成员 br0` 勾选所有网口：

![CqmR8F_Bm2cnU](https://slark-blog.s3.bitiful.net/CqmR8F_Bm2cnU.png)
